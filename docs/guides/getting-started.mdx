...
At its core, LiveKit is a WebRTC SFU. This means it acts as a WebRTC peer and maintains connections with each participant in a room. Each client participant connects to the LiveKit server, not to each other.
