...
Room APIs are built on top of Twirp, and differ from a traditional REST interface. Arguments are passed via POSTing JSON body to the endpoints. The APIs will be servied at `/twirp/livekit.RoomService/<MethodName>`. For example, ListRooms is available at `/twirp/livekit.RoomService/ListRooms`
