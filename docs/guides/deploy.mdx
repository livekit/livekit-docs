...
Deploying WebRTC servers can be tricky, especially on cloud environments. This guide will help you get a secure LiveKit deployment up and running.
...
If you are using TURN (see below), then a separate TURN domain & SSL cert will be needed as well. This is because in order to use TURN/TLS correctly, it'll also need to listen on port 443.
...
Certain corporate firewalls block not only traffic going over UDP, but also non-secure TCP traffic. In those cases, it's helpful to use a TURN server. [Here's](https://bloggeek.me/webrtc-turn/) a good resource if you are interested in reading more about how TURN is used.

The good news is LiveKit includes an embedded TURN server. We have a secure TURN/TLS implementation that has integrated authentication with the rest of LiveKit. This ensures that only clients that are authenticated to join the room can connect to TURN.
