...
Deploying WebRTC servers can be tricky, especially on cloud environments. While LiveKit simplifies some aspects of the deployment, there are a few things to understand.
...
You will also need to set up HTTPS/SSL termination with a load balancer or a reverse proxy.

If you are using TURN (see below), then a separate TURN domain & SSL cert will be needed as well. Since the TURN server would need to be deployed to port 443 as well, you cannot use the same domain as the primary LiveKit endpoint.
...
Certain corporate firewalls block not only traffic going over UDP, but also non-secure TCP traffic. In those cases, it's helpful to use the embedded TURN server within LiveKit. [Here's](https://bloggeek.me/webrtc-turn/) a good resource if you are interested in reading more about how TURN is used.
...
With TURN/TLS, LiveKit will perform the termination, so you will have to specify the certificates in the LiveKit configuration. When running multiple LiveKit instances, you can place a layer 4 load balancer in front of it.
...
When hosting in cloud environments, the ports configured above will have to be opened on the firewall.
...
Open `Inbound rules` tab and select `Edit Inbound Rules`

![AWS inbound rules](/img/deploy/aws-inbound-rules.png)

Then add the following rules (assuming using default ports)
...
Navigate to VPC network, then select `Firewall` on the left. Then select `Create Firewall Rule` on the top menu.
