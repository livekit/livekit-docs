...
LiveKit participants have a `metadata` field that you could use to store application-specific metadata for each participant. For example, this could be the seat number around a table, or the other state.
...
We suggest sending small chunks of data (under 1.4kB) to ensure that the packet doesn't exceed network MTU.
...
```
The React SDK has a peer dependency on the JS SDK, for publishing and other interactions, refer to the JS SDK.
