...
Currently, simulcasting is currently only available in browser-based SDKs.

With simulcasting, a client publishes multiple versions of the same video track with varying resolutions. This allows LiveKit to dynamically forward the stream that's most appropriate given each receiving participant's available bandwidth. This process happens in automatically on the server side when the server detects a participant is bandwidth constrainted. When the participant's bandwidth improves, the server will then upgrade the stream to a higher resolution.
