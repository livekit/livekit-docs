...
It's common to publish the local participant's camera and/or microphone streams in a conference.
...
Disabling camera or microphone will turn off the respective recording indicators. Other participants will receive a `TrackMuted` event.
...
The Go SDK makes it simple to publish static files or media from other sources to the room.

To publish files, they need to be first [encoded](https://github.com/livekit/server-sdk-go/tree/main#publishing-tracks-to-room) into the right format.
...
With Flutter SDK, audio session is managed automatically.
...
On iOS, LiveKit provides automatic management of `AVAudioSession`. We ensure that the minimal set of audio permissions are acquired (for example, microphone is turned off when it's not publishing).
...
Adaptive layer selection takes place automatically in the SFU when the server detects that a participant is bandwidth constrainted. When the participant's bandwidth improves, the server will then upgrade the stream to a higher resolution.
...
Simulcast is supported in all of LiveKit's client SDKs. You can enable them in the default publish settings or when publishing video tracks.
