...
Participants within the room may also [exchange data](room/data#data-messages) with one another, including one-to-one and one-to-many deliveries.

Rooms may be manually created with [server APIs](server-api), or automatically created when the first participant attempts to join a room. When the last remaining participant leaves a room, it will be closed after a short delay.
