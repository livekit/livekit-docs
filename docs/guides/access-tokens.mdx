...
When LiveKit clients connect to the server, they have to pass in an access tokenThe token encodes identity of the participant, name of the room, as well as capabilities & permissions. Access tokens are JWT based and are signed with API secret to prevent forgery.
...
<Tabs
  ...
  groupId="server-sdk"
  values={[
    {label: 'Node (TypeScript)', value: 'typescript'},
