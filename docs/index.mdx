...
We built LiveKit because we believe there should be an open source, production-grade implementation of audio/video rooms that is accessible to all, with a lower barrier of entry in cost and technical complexity. LiveKit provides an opinionated, end-to-end implementation of multi-user conferencing with first-party clients for all major platforms.

## Architecture

LiveKit is written in Go. It's made possible because of [pion](https://github.com/pion/webrtc), [ion-sfu](https://github.com/pion/ion-sfu), and most importantly, the amazing community behind them.

LiveKit is designed with the ease of deployment in mind: it's scalable horizontally by just adding nodes. You can run LiveKit on one or 100 nodes with identical configuration; they can route node to node to ensure clients in the same room are connected to the same node.
