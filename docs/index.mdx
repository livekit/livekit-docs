...
- Modern, full-featured [client SDKs](references/client-sdks.md) for JavaScript, Swift, and Kotlin (React, React Native, Unity/C# and Flutter in development)
- Built for production - JWT authentication and [server APIs](references/server-apis.md)
- Robust networking & connectivity over UDP and TCP
- Easy to deploy: one binary and just three ports to forward
- Advanced features like simulcasting, selective subscription, and moderation APIs.
...
While hosted solutions like Twilio and Agora exist, they're cost-prohibitive, have limited flexibility and as proprietary products, create vendor lock-in. Other open source solutions also exist, but they have a steep learning curve and are daunting to customize and deploy.

We built LiveKit because we believe developers should have a free, open source implementation of WebRTC rooms that "just works". LiveKit provides an opinionated, end-to-end implementation of multi-user conferencing with first-party clients for all the major software platforms.

## Architecture

LiveKit is written in Go. It's made possible by [Pion](https://github.com/pion/webrtc), [ion-sfu](https://github.com/pion/ion-sfu), and the amazing communities behind them both.
...
When deployed, LiveKit's only external dependency is Redis.
