...
While there are hosted solutions available like Twilio and Agora. They can be cost prohibitive and being proprietary products, create vendor lock-in. Other open source solutions also exist, but they are typically written in C/C++, and can be daunting to customize and deploy.

We built LiveKit because we believe there should be an open source, production-grade implementation of audio/video rooms that is accessible to all, without barrier of entry in high cost or technical complexity. LiveKit provides an opinionated, end-to-end implementation of multi-user conferencing with first-party clients for all major platforms.

## Architecture

LiveKit is written in Go. It's made possible because of [Pion](https://github.com/pion/webrtc), an excellent WebRTC implementation in Go.

LiveKit is designed with the ease of deployment in mind: it's scalable horizontally by just adding nodes. You can run LiveKit on one node, or 100 nodes, they can route node to node to ensure clients in the same room are connected to the same node.

The only external dependency is Redis.
