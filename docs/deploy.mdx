...
WebRTC servers are usually tricky to deploy because of their use of UDP ports and needing to advertise its own public IP address.

We've addressed many of these challenges with LiveKit. This guide will help you get a secure LiveKit deployment up and running.
...
If you are using TURN, then a separate TURN domain and SSL cert will be needed, as well. In order to use TURN/TLS correctly, the server needs to listen on port 443.
